...
async function getProperty(id: string, agentId: string) {
  await dbConnect();
  ...
  return {
    ...
    valuation: property.valuation
      ? {
          ...property.valuation,
          valuatedAt: property.valuation.valuatedAt
            ? property.valuation.valuatedAt.toISOString()
            : undefined,
          customFeatures:
            property.valuation.customFeatures?.map((cf: any) => ({
              name: cf.name,
              room: cf.room,
              weight: cf.weight,
            })) || [],
...
export default async function PropertyDetailPage({
  ...
  // Nederlands label voor woningtype (fallback naar originele waarde)
const propertyTypeLabel =
  PROPERTY_TYPE_LABELS[
    property.basicInfo.propertyType as keyof typeof PROPERTY_TYPE_LABELS
    ...
    <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1 text-muted-foreground">
              ...
              <span>
                {property.basicInfo.city}, {property.basicInfo.province}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{propertyTypeLabel}</div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {property.basicInfo.energyLabel}
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-semibold text-gray-900">
            {tValuation('savedValuation')}
          </h2>
          <a href={`/property-valuation?propertyId=${property._id}`}>
            <Button variant="outline" size="sm" className="gap-2">
              <Calculator className="h-4 w-4" />
              {property.valuation?.finalPrice
                ? tValuation('editValuation')
        ...
        {property.valuation?.finalPrice ? (
          <div className="space-y-6">
            ...
            {property.valuation.selectedFeatures &&
              property.valuation.selectedFeatures.length > 0 && (
                <div>
                  <h3 className="text-sm font-medium text-gray-500 mb-3">
                    {tValuation('selectedFeatures')}
                  </h3>
            ...
            {property.valuation.customFeatures &&
              property.valuation.customFeatures.length > 0 && (
                <div>
                  <h3 className="text-sm font-medium text-gray-500 mb-3">
                    Custom Features
                  </h3>
            ...
            {property.valuation.valuatedAt && (
              <div className="pt-4 border-t border-gray-200">
                <div className="flex items-center gap-2 text-sm text-gray-500">
                  ...
                  <span>
                    {tValuation('valuatedOn')}:{' '}
                    {new Date(property.valuation.valuatedAt).toLocaleDateString(
                      'nl-NL',
                      {
                        year: 'numeric',
                        month: 'long',
      ...
      {property.luxuryFeatures &&
        (property.luxuryFeatures.parking?.available ||
          (property.luxuryFeatures.amenities &&
            property.luxuryFeatures.amenities.length > 0)) && (
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <h2 className="text-xl font-semibold text-gray-900 mb-6">
              {tLuxury('title')}
            </h2>
